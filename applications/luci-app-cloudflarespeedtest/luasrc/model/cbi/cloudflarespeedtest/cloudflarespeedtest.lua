...
s:tab("xraytab", translate("Xray"))
if nixio.fs.access("/etc/config/xray") then
	
	o=s:taboption("xraytab", Flag, "xray_enabled",translate("Xray Enabled"))
	o.rmempty=true	

	local xray_server_table = {}
	uci:foreach("xray", "servers", function(s)
		if s.alias then
			xray_server_table[s[".name"]] = "[%s]:%s" % {string.upper(s.protocol or s.type), s.alias}
		elseif s.server and s.server_port then
			xray_server_table[s[".name"]] = "[%s]:%s:%s" % {string.upper(s.protocol or s.type), s.server, s.server_port}
		end
	end)

	local xray_key_table = {}
	for key, _ in pairs(xray_server_table) do
		table.insert(xray_key_table, key)
	end

	table.sort(xray_key_table)

	o = s:taboption("xraytab", DynamicList, "xray_services",
			translate("Xray Servers"),
			translate("Please select a service"))
			
	for _, key in pairs(xray_key_table) do
		o:value(key, xray_server_table[key])
	end
	o:depends("xray_enabled", 1)
	o.forcewrite = true
